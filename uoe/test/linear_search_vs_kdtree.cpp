...
int main(int argc, char const* argv[]) {
    ...
    for (size_t i = 0; i < n_points; ++i) {
        points[i] =
            (uoe::utils::random::sample_random_point_inside_unit_sphere(direction, 0) * r + bounding_sphere_center);
    }
        ...
        [&]() {
            ...
            auto t1 = std::thread([&]() {
                std::for_each(points.begin(), points.begin() + points.size() / 2, [&](const auto& point) {
                    auto distance = uoe::utils::squared_distance(query_point, point);
            ...
            //     auto distance = uoe::utils::squared_distance(query_point, p);
    ...
    //     measure([&]() {
    //         auto query_point =
    //             uoe::utils::random::sample_random_point_inside_unit_sphere(direction, 0) * r +
