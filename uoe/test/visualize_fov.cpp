...
using uoe::VoxelStatus;

auto main(int argc, char* argv[]) -> int {
    ros::init(argc, argv, "visualize_raycast");
    auto nh = ros::NodeHandle();
    auto publish_rate = ros::Rate(uoe::utils::DEFAULT_LOOP_RATE * 10);
    ...
    auto pub_visualize_marker = [&nh] {
        ...
        return nh.advertise<visualization_msgs::Marker>(topic_name, uoe::utils::DEFAULT_QUEUE_SIZE);
    }();
    ...
    auto ocmap = uoe::Octomap(resolution);
    ...
    const auto visualize_fov = [&](const Position& pos, const Position& target) {
        ...
        const auto plane_border = [&](int i, int j) {
            ...
            // border
            {
                ...
                std::cout << "near:" << '\n';
                // std::cout << uoe::types::yaml(near) << '\n';
                std::cout << fmt_vec3(near) << '\n';
                std::cout << "far:" << '\n';
                // std::cout << uoe::types::yaml(far) << '\n';
        ...
        const auto bbx = uoe::compute_bbx(fov);
        visualize_bbx(bbx);
        ...
        const auto visualize_voxels_inside_fov = [&](const BBX& bbx) {
            ...
            auto cube_msg_gen = uoe::utils::rviz::cube_msg_gen{resolution};
