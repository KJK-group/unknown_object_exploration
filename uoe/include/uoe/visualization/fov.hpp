...
auto visualize_voxels_inside_fov(const FoV& fov, const uoe::Octomap& ocmap, float resolution,
    ...
    static auto cube_msg_gen = uoe::utils::rviz::cube_msg_gen{resolution};
    ...
    ocmap.iterate_over_bbx(bbx, [&](const auto& pt, double, VoxelStatus vs) {
        ...
        if (fov.inside_fov(v) && visible(v)) {
            ...
            // msg.color = uoe::utils::rviz::RGBA{0, 0, 0, 0.4};
            msg.color.r = 0;
            msg.color.g = 0;
            msg.color.b = 0;
            msg.color.a = 0.4;
