...
Mission::Mission(ros::NodeHandle& nh, ros::Rate& rate, types::vec3 target, float timeout,
    ...
    : inspection_complete_(false),
      ...
      duration_(0) {
    // publishers
    pub_mission_state_ = nh.advertise<uoe_msgs::MissionStateStamped>("/uoe/mission/state",
                                                                     utils::DEFAULT_QUEUE_SIZE);
    pub_visualise_ = nh.advertise<visualization_msgs::MarkerArray>("/uoe/mission/visualisation",
                                                                   utils::DEFAULT_QUEUE_SIZE);
    ...
    sub_controller_state_ = nh.subscribe<uoe_msgs::ControllerStateStamped>(
        "/uoe/controller/state", utils::DEFAULT_QUEUE_SIZE, &Mission::controller_state_cb_, this);
    sub_odom_ = nh.subscribe<nav_msgs::Odometry>(
        "/mavros/local_position/odom", utils::DEFAULT_QUEUE_SIZE, &Mission::odom_cb_, this);
    sub_object_completion_ = nh.subscribe<uoe_msgs::ObjectMapCompleteness>(
        "/uoe/object_map_percentage_completion/object_voxel_map/octomap_binary",
    ...
    client_nbv_ = nh.serviceClient<uoe_msgs::NBV>("/uoe/rrt_service/nbv");
    client_rrt_ = nh.serviceClient<uoe_msgs::RrtFindPath>("/uoe/rrt_service/find_path");

    // state
    state_.header.frame_id = uoe::utils::FRAME_WORLD;
    ...
    ros::param::get("/uoe/experiment", experiment_param);
...
auto Mission::controller_state_cb_(const uoe_msgs::ControllerStateStamped::ConstPtr& error)
    -> void {
    controller_state_ = *error;
}
auto Mission::odom_cb_(const nav_msgs::Odometry::ConstPtr& odom) -> void { drone_odom_ = *odom; }

auto Mission::object_map_completion_cb_(const uoe_msgs::ObjectMapCompleteness::ConstPtr& object)
    -> void {
    object_map_ = *object;
}
...
auto Mission::visualise_() -> void {
    ...
    m.id = -2;
    m.pose.position = uoe::utils::transform::vec_to_geometry_msg_point(closest_position_);
...
auto Mission::find_path_(Eigen::Vector3f start, Eigen::Vector3f end)
    ...
    ROS_INFO_STREAM("Requesting RRT path from "
                    ...
                    << "[" << end.x() << "," << end.y() << "," << end.z() << "]");
    auto rrt_msg = uoe_msgs::RrtFindPath{};

    auto drone_param = std::map<std::string, double>();
    ros::param::get("/uoe/drone", drone_param);
    ...
    ros::param::get("/uoe/rrt/params", rrt_param);
    if (rrt_param.empty()) {
...
auto Mission::find_nbv_path_(Eigen::Vector3f start) -> std::optional<std::vector<Eigen::Vector3f>> {
...

    {
        ...
        ros::param::get("/uoe/nbv", nbv_param);
        if (nbv_param.empty()) {
        ...
        for (const auto& ip : interest_points_) {
            nbv.excluded_points.emplace_back(uoe::utils::transform::vec_to_geometry_msg_point(ip));
        }
...

    {
        ...
        ros::param::get("/uoe/camera", camera_param);
...
auto Mission::trajectory_step_(float vel, bool look_forwards) -> bool {
    ...
    // std::cout << uoe::utils::GREEN << "distance: " << distance << uoe::utils::RESET << std::endl;
    // std::cout << uoe::utils::GREEN << "remaining_distance: " << remaining_distance
    //           << uoe::utils::RESET << std::endl;
    // std::cout << uoe::utils::GREEN << "ERRORNORM: " << controller_state_.error.position.norm
    //           << uoe::utils::RESET << std::endl;
...
auto Mission::set_test_trajectory_() -> void {
    ROS_INFO_STREAM(uoe::utils::GREEN << "Setting PID testing trajectory");
...
auto Mission::run_step() -> void {
    ...
    // std::cout << uoe::utils::GREEN << "completion: " << object_map_.percentage << "/"
    //           << percentage_threshold << uoe::utils::RESET << std::endl;
    // std::cout << uoe::utils::MAGENTA
        ...
        case HOME:
            if (inspection_complete_) {
                ...
                if (! takeoff_initiated_) {
                    ...
                    // std::cout << uoe::utils::MAGENTA << "PID_TEST1" << uoe::utils::RESET
                    //           << std::endl;
                    // std::cout << uoe::utils::MAGENTA << state_to_string((state)state_.state)
                    //           << uoe::utils::RESET << std::endl;
#endif
                ...
                if (trajectory_step_(velocity_target_, true)) {
                    ...
                    // std::cout << uoe::utils::MAGENTA << "PID_TEST" << uoe::utils::RESET
                    //           << std::endl;
                    // std::cout << uoe::utils::MAGENTA << state_to_string((state)state_.state)
                    //           << uoe::utils::RESET << std::endl;
                }
    ...
    closest_position_ = trajectory_.get_closest_point(
        uoe::utils::transform::geometry_mgs_point_to_vec(drone_odom_.pose.pose.position));
    publish_();
}
...
auto Mission::end() -> void {
    ...
    if (state_.state == LAND &&
        drone_odom_.pose.pose.position.z < uoe::utils::SMALL_DISTANCE_TOLERANCE) {
        ros::shutdown();
        return;
    } else {
        state_.state = HOME;
