...
class kdtree3 {
    struct Node {
        ...
        inline auto xyz() { return std::make_tuple(position_.x(), position_.y(), position_.z()); }
    };
   ...
   public:
    ...
    //   }
    auto contains(const Eigen::Vector3f& value) -> bool { return contains(root_, value, 0); }
    /**
    ...
    //     return knn_search(root_, k, node, std::vector<Eigen::Vector3f>());
    // }
    // auto approximate_knn_search(const Eigen::Vector3f& node,
    //                             unsigned int limit_of_nodes_to_compare_with, unsigned int k = 1)
    //     -> std::vector<Eigen::Vector3f> {
    //     return knn_search(root_, k, node);
    // }
    auto range_search(range x_range, range y_range, range z_range) -> std::vector<Eigen::Vector3f>;
    ...
    auto range_search_in_xy(range x_range, range y_range) -> std::vector<Eigen::Vector3f>;
    auto range_search_in_xz(range x_range, range z_range) -> std::vector<Eigen::Vector3f>;
    auto range_search_in_yz(range y_range, range z_range) -> std::vector<Eigen::Vector3f>;

    auto max_depth() -> unsigned int;
    // ! do not use
    auto is_balanced() -> bool;

    auto inorder_traversal(std::function<void(const Eigen::Vector3f&)> fn) -> void {
        inorder_traversal(root_, fn);
    }

    auto inorder_traversal(std::function<void(const Eigen::Vector3f&, unsigned int)> fn) -> void {
        inorder_traversal(root_, fn, 0);
    }

    auto preorder_traversal(std::function<void(const Eigen::Vector3f&)> fn) -> void {
        preorder_traversal(root_, fn);
    }
    auto postorder_traversal(std::function<void(const Eigen::Vector3f&)> fn) -> void {
        postorder_traversal(root_, fn);
    }

    auto bfs(std::function<void(const Eigen::Vector3f&)> fn) -> void { bfs(root_, fn); }

   private:
    auto insert(std::unique_ptr<Node>& node, const Eigen::Vector3f& value, unsigned int depth)
        -> void;
    ...
    auto postorder_traversal(std::unique_ptr<Node>& node,
                             std::function<void(const Eigen::Vector3f&)> fn) -> void;
    auto bfs(std::unique_ptr<Node>& node, std::function<void(const Eigen::Vector3f&)> fn) -> void;

    auto knn_search(std::unique_ptr<Node>& node, unsigned int k, const Eigen::Vector3f& value,
                    std::vector<Eigen::Vector3f>& result) -> std::vector<Eigen::Vector3f>;

    auto contains(std::unique_ptr<Node>& node, const Eigen::Vector3f& value, unsigned int depth)
        -> bool;
