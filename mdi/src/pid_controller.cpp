...
auto angular_pid_velocities(geometry_msgs::Quaternion target_attitude) -> float {
    ...
    // pid output
    auto y_vel = kap * error_heading + kai * error_yaw_integral + kad * error_yaw_derivative;
...
auto visualise() -> void {
    ...
    // auto m3 = arrow_builder.arrow_head_width(0.05)
    //               .arrow_width(0.025)
    //               .arrow_length(0.05)
    //               .color(0, 0, 1, 1)
    //               .build()({pos, pos + c_tf});
    // m3.header.frame_id = mdi::utils::FRAME_WORLD;
    // m3.id = -11;
    // pub_visualisation.publish(m3);
}
...
auto main(int argc, char** argv) -> int {
    ...
    pub_visualisation = nh.advertise<visualization_msgs::Marker>("/mdi/visualisation/controller", 10);
