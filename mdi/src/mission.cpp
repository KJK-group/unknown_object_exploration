...
namespace mdi {
Mission::Mission(ros::NodeHandle& nh, ros::Rate& rate, types::vec2 target, float velocity_target, Eigen::Vector3f home,
                 bool visualise)
    : inspection_complete_(false),
      ...
      timeout_({5}) {
    // publishers
    pub_mission_state_ = nh.advertise<mdi_msgs::MissionStateStamped>("/mdi/mission/state", utils::DEFAULT_QUEUE_SIZE);
    pub_visualise_ = nh.advertise<visualization_msgs::Marker>("/mdi/mission/visualisation", utils::DEFAULT_QUEUE_SIZE);
    pub_setpoint_ = nh.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_local/local", utils::DEFAULT_QUEUE_SIZE);

    // subscribers
    sub_drone_state_ =
        nh.subscribe<mavros_msgs::State>("/mavros/state", utils::DEFAULT_QUEUE_SIZE, &Mission::mavros_state_cb_, this);
    ...
    sub_odom_ = nh.subscribe<nav_msgs::Odometry>("/mavros/local_position/odom", utils::DEFAULT_QUEUE_SIZE,
                                                 &Mission::odom_cb_, this);
...
auto Mission::mavros_state_cb_(const mavros_msgs::State::ConstPtr& state) -> void { drone_state_ = *state; }
auto Mission::controller_state_cb_(const mdi_msgs::ControllerStateStamped::ConstPtr& error) -> void {
...
auto Mission::add_interest_point(Eigen::Vector3f interest_point) -> void { interest_points_.push_back(interest_point); }
...
auto Mission::drone_takeoff(float altitude) -> bool {
    ...
    start_time_ = ros::Time::now() - ros::Duration(utils::DEFAULT_DISTANCE_TOLERANCE / velocity_target_);
...
auto Mission::find_path_(Eigen::Vector3f start, Eigen::Vector3f end) -> std::vector<Eigen::Vector3f> {
...
auto Mission::fit_trajectory_(std::vector<Eigen::Vector3f> path) -> std::optional<trajectory::CompoundTrajectory> {
...
auto Mission::exploration_step_() -> bool {
    ...
    if (step_count_ == 0 || (remaining_distance < utils::SMALL_DISTANCE_TOLERANCE &&
        ...
        if (waypoint_idx_ >= interest_points_.size()) {
            ...
            auto start = interest_points_[waypoint_idx_ - 1];
            auto end = interest_points_[waypoint_idx_];
            auto path = find_path_(start, end);
...
/**
 * @brief updates the expected position to the expect distance travelled at this time with the given velocity
 *
...
auto Mission::trajectory_step_() -> bool {
    ...
    std::cout << mdi::utils::GREEN << "ERRORNORM: " << controller_state_.error.position.norm << mdi::utils::RESET
              << std::endl;
